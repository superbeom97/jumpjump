...
def Start_Student(json_big_data):
    while True:
        ...
        if initial_number == 1:      ## 학생 정보 입력
            ...
            if select_number == 1:
                search_student =
                Total_Student_Info_Print(search_student, json_big_data, search_info, now_course_info)
...
def Total_Student_Info_Print(search_student, json_big_data, search_info, now_course_info):        ## 학생 정보 조회 출력 함수 1-2
    for total_pr in json_big_data:
        Student_Info_Print(search_student, json_big_data, search_info, now_course_info, total_pr)

def Student_Info_Print(search_student, json_big_data, search_info, now_course_info, total_pr):
