...
def Make_Weather_Json(day_time):     ## (1) 기상 정보(동네예보정보 조회 서비스) JSon_ 파일 생성하는 함수
    ...
    with open('동구_신암동_초단기예보조회_%s.JSon_' % yyyymmdd, 'w', encoding='utf8') as outfile:
        retJson = json.dumps(json_weather_result, indent=4, sort_keys=True, ensure_ascii=False)

        outfile.write(retJson)

    print('동구_신암동_초단기예보조회_%s_%s.JSon_ SAVED\n' % (yyyymmdd, day_time))
...
def Read_Weather_Json():        ## (1) 기상 정보(동네예보정보 조회 서비스) JSon_ 파일을 불러오는(읽는) 함수
    total_weather = []
    with open("동구_신암동_초단기예보조회_%s.JSon_" % yyyymmdd, encoding='UTF8') as json_file:
...
def Make_Atmosphere_Json():  ## (2) 통합대기환경 정보(대기오염정보 조회 서비스) JSon_ 파일 생성하는 함수
    ...
    with open('동구_신암동_통합대기환경정보_%s.JSon_' % yyyymmdd, 'w', encoding='utf8') as outfile:
        retJson = json.dumps(json_atmosphere_result, indent=4, sort_keys=True, ensure_ascii=False)

        outfile.write(retJson)

    print('동구_신암동_통합대기환경정보_%s.JSon_ SAVED\n' % yyyymmdd)

def Read_Atmosphere_Json():        ## (2) 통합대기환경 정보(대기오염정보 조회 서비스) JSon_ 파일을 불러오는(읽는) 함수
    total_atmosphere = []
    with open("동구_신암동_통합대기환경정보_%s.JSon_" % yyyymmdd, encoding='UTF8') as json_file:
        json_object = json.load(json_file)
        json_string = json.dumps(json_object)
...
def Food_recommendation():      ## 요리 레시피 / 맛집 추천해 주는 함수
    ...
    local_food = urllib.parse.quote("신암동 ")
    want_food = urllib.parse.quote(inp_want_food)  ## 신암동 요리 맛집 JSon_ 파일 request 날릴 때, 항목
    restaurant_food = urllib.parse.quote(" 맛집")  ## 신암동 요리 맛집 JSon_ 파일 request 날릴 때, 항목
    ...
    print("")
    print("<< 신암동 %s 맛집 추천 목록입니다:) 관심 있는 맛집은 바로 가기를 클릭해 주세요~ >>".center(55) % inp_want_food)
