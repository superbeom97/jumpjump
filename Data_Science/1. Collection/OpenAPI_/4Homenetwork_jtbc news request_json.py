...
try:
    ...
    with open('%s_facebook_%s_%s.JSon_' % (page_name, from_date, range_date), 'w', encoding='utf8') as outfile:
        str_ = json.dumps(jsonResult, indent=4, sort_keys=True, ensure_ascii=False)
        outfile.write(str_)
        print('%s_facebook_%s_%s.JSon_ SAVED' % (page_name, from_date, range_date))
