...
def get_realtime_weather_info():
    ...
    if 30 < day_min_int <= 59:      ## 실시간 업데이트가 있는지 없는지 확인,, 30분부터 59분까지는 실시간 정보 업데이트 됨
        ...
        with open('동구_신암동_초단기예보조회_%s_%s.JSon_' % (yyyymmdd, day_time), 'w', encoding='utf8') as outfile:
            retJson = json.dumps(jsonResult, indent=4, sort_keys=True, ensure_ascii=False)

            outfile.write(retJson)

        print('동구_신암동_초단기예보조회_%s_%s.JSon_ SAVED' % (yyyymmdd, day_time))
...
def Devices_AI():
    ...
    total_weather = []      ## 인공지능 모드를 위해 JSon_ 파일의 정보를 읽어 오는
    with open("동구_신암동_초단기예보조회_%s_%s.JSon_" % (yyyymmdd, day_time), encoding='UTF8') as json_file:
