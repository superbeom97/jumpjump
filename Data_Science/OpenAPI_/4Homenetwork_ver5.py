...
##        JSon_ 파일을 불러오는(읽는) 코드를 함수로 묶음
##        제습기 작동 범위 55 <= humidity_status_num <= 70 추가
##        기온을 받아 난방기 or 에어컨 작동
##        함수 이름 정리
##        통합대기환경 정보(대기오염정보 조회 서비스) JSon_ 파일로 불러오기 -> 통합대기환경 / 미세먼지 정보 이용
...
def Make_Atmosphere_Json():  ## (2) 통합대기환경 정보(대기오염정보 조회 서비스) JSon_ 파일 생성하는 함수
    ...
    print('동구_신암동_통합대기환경정보_%s.JSon_ SAVED' % yyyymmdd)
...
def Devices_AI():   ## 실시간 업데이트 정보 request -> JSon_ 파일 만들고 -> 장비 제어하는 함수
    ...
    if 30 < get_Realtime_Weather_Info() <= 59:  ## 실시간 업데이트가 있는지 없는지 확인,, 30분부터 59분까지는 실시간 정보 업데이트 됨
        ...
        total_weather = Read_Weather_Json()  ## JSon_ 파일을 불러오는 함수 - 인공지능 모드를 위해 JSon_ 파일의 정보를 읽어 오는
