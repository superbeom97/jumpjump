...
## [HK Lee Comment] 특정시간대에 결함이 발생할 수 있는 요소 점검
## 레시피 정보는 -> 냉장고 디바이스를 만들어서 표시
## 뉴스 / TV / Music 검색정보를 수신하는 디바이스를 선택하여 표시할 수 있도록 추천
## -> TV로 볼 것인지, 스마트폰으로 볼 것인지, 스피커로 들을 것인지
## 창문은 소나기의 변수가 있으니, 인공지능 모드시 닫는 용도로만
## 1,000 라인이 넘어 가면 모듈로 나눠서 관리하기


## 난방기 / 에어컨의 경우, 실내 내부의 온도를 인지할 센서가 없어,
## 시뮬레이션을 위해 외부의 온도(실시간 기상 정보)를 끌어와 사용
...
last_thrid = "30"       ## (1) 기상 정보(동네예보정보 조회 서비스) JSon_ 파일 request 날릴 때, 항목
######## x_coodinate / y_coodinate는 기상 정보를 알아볼 위치 89 /91은 신암동
x_coodinate = "89"      ## (1) 기상 정보(동네예보정보 조회 서비스) JSon_ 파일 request 날릴 때, 항목
y_coodinate = "91"      ## (1) 기상 정보(동네예보정보 조회 서비스) JSon_ 파일 request 날릴 때, 항목
numofrows = "100"       ## (1) 기상 정보(동네예보정보 조회 서비스) JSon_ 파일 request 날릴 때, 항목
sidoname = "대구"       ## (2) 통합대기환경 정보(대기오염정보 조회 서비스) JSon_ 파일 request 날릴 때, 항목
stationname = "신암동"  ## (2) 통합대기환경 정보(대기오염정보 조회 서비스) JSon_ 파일 request 날릴 때, 항목
ver_info = "1.3"        ## (2) 통합대기환경 정보(대기오염정보 조회 서비스) JSon_ 파일 request 날릴 때, 항목
...
restaurant_local = "신암동"       ## "신암동" - 신암동 요리 맛집 JSon_ 파일 request 날릴 때, 항목
good_restaurant = "맛집"          ## "맛집" - 신암동 요리 맛집 JSon_ 파일 request 날릴 때, 항목
...
def Make_Weather_Json(day_time):     ## (1) 기상 정보(동네예보정보 조회 서비스) JSon_ 파일 생성하는 함수
    ...
    with open('동구_%s_초단기예보조회_%s.JSon_' % (stationname, yyyymmdd), 'w', encoding='utf8') as outfile:
        retJson = json.dumps(json_weather_result, indent=4, sort_keys=True, ensure_ascii=False)

        outfile.write(retJson)

    print('동구_%s_초단기예보조회_%s_%s.JSon_ SAVED\n' % (stationname, yyyymmdd, day_time))

def get_Realtime_Weather_Info():        ## (1) 기상 정보(동네예보정보 조회 서비스) JSon_ 파일 만들기 전, 실시간 업데이트 확인 함수
...
def Read_Weather_Json():        ## (1) 기상 정보(동네예보정보 조회 서비스) JSon_ 파일을 불러오는(읽는) 함수
    total_weather = []
    with open("동구_%s_초단기예보조회_%s.JSon_" % (stationname, yyyymmdd), encoding='UTF8') as json_file:
...
def get_Atmosphere_URL():   ## (2) 통합대기환경 정보(대기오염정보 조회 서비스) request 보내기 전, url 만드는 함수
    ...
    parameters = "?_returnType=JSon_&serviceKey=" + access_key
...
def Make_Atmosphere_Json():  ## (2) 통합대기환경 정보(대기오염정보 조회 서비스) JSon_ 파일 생성하는 함수
    ...
    with open('동구_%s_통합대기환경정보_%s.JSon_' % (stationname, yyyymmdd), 'w', encoding='utf8') as outfile:
        retJson = json.dumps(json_atmosphere_result, indent=4, sort_keys=True, ensure_ascii=False)

        outfile.write(retJson)

    print('동구_%s_통합대기환경정보_%s.JSon_ SAVED\n' % (stationname, yyyymmdd))

def Read_Atmosphere_Json():        ## (2) 통합대기환경 정보(대기오염정보 조회 서비스) JSon_ 파일을 불러오는(읽는) 함수
    total_atmosphere = []
    with open("동구_%s_통합대기환경정보_%s.JSon_" % (stationname, yyyymmdd), encoding='UTF8') as json_file:
        json_object = json.load(json_file)
        json_string = json.dumps(json_object)
...
def Devices_AI():   ## 실시간 업데이트 정보 request -> JSon_ 파일 만들고 -> 장비 제어하는 함수
    get_Realtime_Weather_Info()
    print("\n<<가장 최신 대기환경정보 업데이트를 실시합니다!!>>\n".center(30))
    Make_Atmosphere_Json()
    total_weather = Read_Weather_Json()  ## 기상정보 JSon_ 파일을 불러오는 함수 - 인공지능 모드를 위해 JSon_ 파일의 정보를 읽어 오는
    total_atmosphere = Read_Atmosphere_Json()  ## 대기환경정보 JSon_ 파일을 불러오는 함수 - 인공지능 모드를 위해 JSon_ 파일의 정보를 읽어 오는
...
def Smart_Mode():       ## 스마트 모드 메뉴 함수
    ...
    if menu_num == 1:       ## 인공지능 모드 조회
        ...
        if g_AI_Mode == True:
            total_weather = Read_Weather_Json()  ## 기상정보 JSon_ 파일을 불러오는 함수
            total_atmosphere = Read_Atmosphere_Json()   ## 대기환경통합정보 JSon_ 파일을 불러오는 함수
        ...
        if g_AI_Mode == True:
            total_weather = Read_Weather_Json()  ## 기상정보 JSon_ 파일을 불러오는 함수
            total_atmosphere = Read_Atmosphere_Json()  ## 대기환경정보 JSon_ 파일을 불러오는 함수
...
def Update_Json():  ## 따옴표가 <b> </b>로 출력돼서 다시 따옴표로 바꿔주는 작업 + 링크 주소가 잘못 나와서 다시 조합하는 작업 함수
    food_blog_ls = []
    with open('%s_%s_%s.JSon_' % (inp_want_food, item_type, yyyymmdd), encoding='UTF8') as json_file:
        json_object = json.load(json_file)
        json_string = json.dumps(json_object)
    ...
    with open('%s_%s_%s.JSon_' % (inp_want_food, item_type, yyyymmdd), 'w', encoding='utf8') as outfile:
...
def Recipe_Request_Json(url):       ## 요리 레시피 / 맛집을 당겨오기 위해 request 보내는 함수
    ...
    if rescode == 200:
        ...
        with open('%s_%s_%s.JSon_' % (inp_want_food, item_type, yyyymmdd), 'w', encoding='utf8') as outfile:
            ...
            outfile.write(retJson)
            print('\n%s_%s_%s.JSon_ SAVED' % (inp_want_food, item_type, yyyymmdd))
...
def Food_recommendation():      ## 요리 레시피 / 맛집 추천해 주는 함수
    ...
    ########################### 레시피 추천
    item_type = "recipe"
    want_food = urllib.parse.quote(inp_want_food)  ## 요리 레시피 JSon_ 파일 request 날릴 때, 항목
    recipe_food = urllib.parse.quote(" 레시피")  ## 요리 레시피 JSon_ 파일 request 날릴 때, 항목
    research_food = want_food + recipe_food  ## 요리 레시피 JSon_ 파일 request 날릴 때, 항목

    get_Recipe_URL()        ## 요리 레시피를 당겨오기 위해 request 보내는 전, URL 만드는 함수

    recipe_blog_ls = Update_Json()      ## JSon_ 파일에서 레시피 목록을 가져오는 변수
    print("")
    ...
    local_food = urllib.parse.quote(restaurant_local)         ## "신암동" - 신암동 요리 맛집 JSon_ 파일 request 날릴 때, 항목
    want_food = urllib.parse.quote(inp_want_food)             ## 신암동 요리 맛집 JSon_ 파일 request 날릴 때, 항목
    restaurant_food = urllib.parse.quote(good_restaurant)     ## "맛집" - 신암동 요리 맛집 JSon_ 파일 request 날릴 때, 항목
    research_food = local_food + want_food + restaurant_food  ## 신암동 요리 맛집 JSon_ 파일 request 날릴 때, 항목
    ...
    restaurant_blog_ls = Update_Json()      ## JSon_ 파일에서 맛집 목록을 가져오는 변수
...
def Simulation_Mode():      ## 시뮬레이션 모드 메뉴
    ...
    get_Realtime_Weather_Info()     ## JSon_ 파일 만들기 전 함수

    total_weather = Read_Weather_Json()     ## JSon_ 파일을 불러오는 함수
