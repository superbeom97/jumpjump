###################################### 네이버 검색 Open API - 요리 레시피 추천 - 네이버 블로그 기반
...
if(rescode==200):
    ...
    with open('%s_recipe_naver_블로그.JSon_' % inp_want_food, 'w', encoding='utf8') as outfile:
        str_ = json.dumps(jsonResult, indent=4, sort_keys=True, ensure_ascii=False)
        outfile.write(str_)
        print('%s_recipe_naver_블로그.JSon_ SAVED' % inp_want_food)

else:
    print("Error Code:" + rescode)

recipe_blog_ls = []
with open("%s_recipe_naver_블로그.JSon_" % inp_want_food, encoding='UTF8') as json_file:
