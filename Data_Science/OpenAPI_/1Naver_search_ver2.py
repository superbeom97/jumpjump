...
## [ver1]   JSon_ 파일에서 따옴표가 &quot;로 출력돼서 다시 따옴표로 바꿔주는 작업
...
def main():
    ...
    while ((jsonSearch != None) and (jsonSearch['display'] != 0) and index < 9):
        ...
        with open('%s_naver_%s.JSon_' % (search_text, sNode), 'w', encoding='utf8') as outfile:
            retJson = json.dumps(jsonResult, indent=4, sort_keys=True, ensure_ascii=False)
            outfile.write(retJson)

        print("%s_naver_%s.JSon_ SAVED" % (search_text, sNode))
        Sort_Print(search_text, sNode)

def Sort_Print(search_text, sNode):
    total_news = []
    with open("%s_naver_%s.JSon_" % (search_text, sNode), encoding='UTF8') as json_file:
