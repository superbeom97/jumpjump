## [ver2(_ver1에서 업데이트)]
## 1. JSon_ 파일을 새로 생성하지만, ID 부여 index txt가 있을 때에도 ITT001부터 부여되게 함
...
## [ver5(_ver4에서 업데이트)]
## JSon_ 파일 읽는 것과 쓰는 것을 함수로 처리

##[ver6(_ver5에서 업데이트)]
## 1. JSon_ 파일 읽고 쓰는 함수 중복 -> 하나로 처리
...
def Start_Student(path_number, json_big_data):
    ...
    except:       ## str 나 공백으로 인한 오류를 잡아주기 위해
        print("입력을 잘못하셨습니다. 다시 입력해 주세요!!\n")
    ...
    while True:
        ...
        if now_course_exist == 'Y' or now_course_exist == 'y':
            ...
            if os.path.isfile("Student_ID_info.txt"):  ## 고유 아이디 생성 후 배정_아이디 배정 txt 파일이 있을 경우
                if path_number == 2:            ## JSon_ 파일을 신규 생성하는데, 아이디 배정 txt 파일이 있으면 001부터가 아닌
                ...
                Read_Json(create_student, json_big_data)  ## JSon_ 파일 읽는 함수로
                Make_Json(json_big_data)        ## JSon_ 파일 생성하는 함수로
...
def Make_Json(json_big_data):       ## JSon_ 파일 생성하는 함수
    with open('ITT_Student.JSon_', 'w', encoding='utf8') as outfile:
...
## Entry Point~~
json_big_data = []
if os.path.isfile("ITT_Student.JSon_"):      ## 프로그램 시작 시 소스코드가 있는 경로에 'ITT_Student.JSon_' 파일을 읽어 들인다
    path_number = 0
    with open("ITT_Student.JSon_", encoding='UTF8') as json_file:
        json_object = json.load(json_file)
        json_string = json.dumps(json_object)
        json_big_data = json.loads(json_string)
        Start_Student(path_number, json_big_data)
elif not os.path.isfile("ITT_Student.JSon_"):        ## 파일이 없을 시
    path_number = int(input("<<파일이 존재하지 않습니다.>>\n1. 경로 선택\n2. 신규 생성\n-> "))
    if path_number == 1:
        path_path = input("파일 경로를 입력해 주세요 : ")
        with open("%s\\ITT_Student.JSon_" % path_path, encoding='UTF8') as json_file:
            json_object = json.load(json_file)
            json_string = json.dumps(json_object)
            json_big_data = json.loads(json_string)
            Start_Student(path_number, json_big_data)
