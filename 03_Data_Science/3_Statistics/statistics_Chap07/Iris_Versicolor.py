...
iris['YesorNo'] = np.where(iris['variety'] == 'Versicolor', 1., 0.)
...
new_observations = iris.ix[iris.index.isin(range(51, 150)), independent_variables.columns]
new_observations_with_constant = sm.add_constant(new_observations, prepend=True)
y_predicted = logit_model.predict(new_observations_with_constant)
# y_predicted_rounded = [round(score) for score in y_predicted]       ## round() : 반올림 해줌
y_predicted_rounded = [round(score, 2) for score in y_predicted]    ## round(score, 2) : 둘째자리까지 반올림 해줌
