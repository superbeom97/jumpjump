...
def Food_recommendation():      ## 요리 레시피 / 맛집 추천해 주는 함수
    ...
    restaurant_food = urllib.parse.quote(" 맛집")  ## 신암동 요리 맛집 json 파일 request 날릴 때, 항목
...
def Response_Video(soup):
    video_num = 0
    today_video_info = soup.findAll('div', attrs={'class':'top_flick'})     ## <div class="flick"> 에 포함되어 있는 소스코드를 가져오는
    today_video_lowrank = soup.findAll('dt', attrs={'class':'title'})     ## <dt class="title"> 에 포함되어 있는 소스코드를 가져오는
    video_title_group = soup.findAll('strong', attrs={'class': 'tit'})      ## <strong class="tit"><span>박시후 X 신혜선, 지하철 타고 영화관 데이트♥</span> 에서 제목 뽑아 내기
    video_link_group = soup.findAll('a')      ## <strong class="tit"><span>박시후 X 신혜선, 지하철 타고 영화관 데이트♥</span> 에서 제목 뽑아 내기

    for z in today_video_lowrank:
        print(z['href'])

    video_title_ls = []
    video_link_ls = []
    # for each_video in today_video_info:
    #     video_num += 1
    #     video_title_ls.append(each_video)
    #     video_link_ls.append(each_video['href'])
    # print("<< %s 위 >>" % news_num)
    # print("제목 : %s" % news_title_link['title'])  ## <li class="num1"> 에 포함되어 있는 <a> 에 포함되어 있는 title 속성을 가져오는
    # print("바로 가기 ☞  %s\n" % news_title_link['href'])  ## <li class="num1"> 에 포함되어 있는 <a> 에 포함되어 있는 href 속성을 가져오는
    #
